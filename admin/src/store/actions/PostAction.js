...
export const createAction = (postData) =>{
    return async(dispatch,getState)=>{
        ...
        dispatch({type: SET_POST_LOADER});
        try {
                const { data:{msg} } = await axiosInstance.post(`/create-post`,postData); 
                dispatch({type: REMOVE_POST_LOADER});
...
export const deleteAction = (id) =>{
    return async (dispatch, getState)=>{
            try {
            const {data} = await axiosInstance.get(`/delete-post/${id}`);
            dispatch({type:SET_POST_LOADER});
...
export const fetchPost = (id) =>{
    return async(dispatch,getState)=>{
          dispatch({type: SET_POST_LOADER});
          try {
                const { data } = await axiosInstance.get(`/edit-post/${id}`);
